...
describe("UNIT TEST: Wishport Contract - mint", () => {
  ...
  it(`should throw error if the message hash signer is not the authedSigner`, async () => {
    const [owner, minter, authedSigner, unauthedSigner] =
      await ethers.getSigners();
    const [wishport, _wish] = await contractDeployer.Wishport({
      owner,
  ...
  `, async () => {
    ...
    const snapshot_id = await ethers.provider.send("evm_snapshot", []);
    {
      ...
      await erc20
        .connect(minter)
        .mint(minter.address, UnitParser.toBigNumber(assetAmount_, decimals));
  ...
  `, async () => {
    ...
    const snapshot_id = await ethers.provider.send("evm_snapshot", []);
    {
      ...
      const before = UnitParser.fromBigNumber(
        await erc20.balanceOf(wishport.address),
        decimals
      );
      ...
      expect(
        UnitParser.fromBigNumber(rewardInfo.amount, decimals)
      ).not.to.equal(assetAmount_);
      expect(UnitParser.fromBigNumber(rewardInfo.amount, decimals)).to.equal(
        after + before
      );
    }
