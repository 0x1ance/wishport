...
import { contractReturnHandler, expectEvent, expectFnReturnChange, expectRevert, ZERO_ADDRESS } from '../../../ethers-test-helpers'
...
describe('UNIT TEST: Wish Contract - tokensOfOwner', () => {
  ...
  `, async () => {
    ...
    await wish.connect(wishport).mint(account.address, tokenIds[0])
    await wish.connect(wishport).mint(account.address, tokenIds[1])
    await wish.connect(wishport).mint(account.address, tokenIds[2])

    const res = contractReturnHandler(await wish.connect(wishport)['tokensOfOwner(address)'](account.address))
  ...
  `, async () => {
    ...
    await wish.connect(wishport).setTransferable(tokenIds[0], true)
    await wish.connect(wishport).setTransferable(tokenIds[2], true)

    const res = contractReturnHandler(await wish.connect(wishport)['tokensOfOwner(address,bool)'](account.address, true))
  ...
  `, async () => {
    ...
    const res = contractReturnHandler(await wish.connect(wishport)['tokensOfOwner(address,bool)'](account.address, false))
