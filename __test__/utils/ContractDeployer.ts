...
export type SoulboundDeploymentConfig = ContractDeploymentBaseConfig &
  SoulhubDeploymentConfig & {
    soulhub?: Soulhub;
  };

export type WishDeploymentConfig = ContractDeploymentBaseConfig &
  SoulhubDeploymentConfig & {
    soulhub?: Soulhub;
    soulhubManager?: SoulhubManager;
    name?: string;
...
export type WishportDeploymentConfig = ContractDeploymentBaseConfig &
...
export class ContractDeployer {
  async TestERC20({ owner }: ContractDeploymentBaseConfig = {}) {
    ...
    return [testerc20, targetOwner] as [TestERC20, SignerWithAddress];
  }
  async TestUSDT({ owner }: ContractDeploymentBaseConfig = {}) {
    ...
    return [testUsdt, targetOwner] as [TestUSDT, SignerWithAddress];
  }
  async SoulhubManager({ owner }: ContractDeploymentBaseConfig = {}) {
    const [defaultOwner] = await ethers.getSigners();
    ...
    return [soulhubManager, targetOwner] as [SoulhubManager, SignerWithAddress];
  }
  async Soulhub({
    owner,
    soulhubManager,
    ...
    const targetOwner = owner ?? defaultOwner;
    const targetSoulhubManager =
      soulhubManager ?? (await this.SoulhubManager({ owner: targetOwner }))[0];
    const soulhub = await contractFactory
      .connect(targetOwner)
      .deploy(name, targetSoulhubManager.address);
  ...
  async Soulbound({
    ...
    const targetSoulhub =
      ...
      (
        await this.Soulhub({
          owner: targetOwner,
          soulhubManager: targetSoulhubManager,
          name,
        })
      )[0];

    const contractFactory = await ethers.getContractFactory("Soulbound", {});
    const soulbound = await contractFactory
      .connect(targetOwner)
      .deploy(targetSoulhub.address);
  ...
  async Wish({
    ...
    const wish = await contractFactory
      .connect(targetOwner)
      .deploy(
        name,
        symbol,
  ...
  async Wishport({
    ...
    return [
      ...
      wish,
      targetSoulhub,
      targetSoulhubManager,
      targetOwner,
    ] as [Wishport, Wish, Soulhub, SoulhubManager, SignerWithAddress];
  }
}
