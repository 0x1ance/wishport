...
abstract contract TokenRecovery is Context, Ownable {
    using SafeERC20 for IERC20;
    using Address for address payable;

    /**
     * @dev Indicates an error related to the length comparison of two arrays.
     */
    error TokenRecoveryInconsistentArrays();
    ...
    function recoverETH(
        ...
        if (length != amounts.length) {
            revert TokenRecoveryInconsistentArrays();
        }
    ...
    function recoverERC721s(
        ...
        uint256 length = accounts.length;
        if (length != contracts.length || length != tokenIds.length) {
            revert TokenRecoveryInconsistentArrays();
        }
        unchecked {
            for (uint256 i; i != length; ++i) {
